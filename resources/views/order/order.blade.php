...
@section('styles')
    ...
    <style>
        ...
        .swal-confirm-button { border: 2px solid #ca8a04 !important; }
    </style>
@endsection
        ...
        <form id="order-form" method="POST" action="{{ route('orders.store') }}">
            ...
            <main class="space-y-12">
                @foreach ($menuCategories as $category => $menus)
                    <div>
                        <h2 class="font-lora text-3xl font-bold border-b-2 border-brand-accent/50 pb-2 mb-8 uppercase">{{ $category }}</h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            @foreach ($menus as $menu)
                                <div class="menu-item bg-brand-card rounded-lg border border-brand-accent/50 shadow-lg overflow-hidden transition-transform duration-300 hover:scale-[1.02]"
                                    data-id="{{ $menu->menu_id }}" data-price="{{ $menu->price }}" data-name="{{ $menu->name }}">
                                    <div class="flex">
                                        ...
                                        <div class="p-4 sm:p-6 flex flex-col justify-between w-2/3">
                                            ...
                                            <div class="mt-auto">
                                                ...
                                                <div class="flex justify-between items-center">
                                                    <p class="text-lg font-bold text-brand-accent">IDR {{ number_format($menu->price, 0, ',', '.') }}</p>
                                                    <div class="flex items-center space-x-2">
                                                        <button type="button" class="quantity-btn minus-btn bg-gray-700 hover:bg-gray-600 rounded-full w-8 h-8 text-lg font-bold transition-colors">-</button>
                                                        <span class="quantity-display text-white font-bold w-8 text-center text-lg">0</span>
                                                        <button type="button" class="quantity-btn plus-btn bg-brand-accent hover:bg-brand-accent-dark rounded-full w-8 h-8 text-lg font-bold text-black transition-colors">+</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
            ...
            <section class="mt-16 bg-brand-card border border-brand-accent/50 p-6 sm:p-8 rounded-lg">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    <div class="space-y-4">
                        ...
                        <div>
                            <label for="order-type" class="block text-sm font-medium text-slate-300 mb-1">Tipe Pesanan</label>
                            <select id="order-type" name="order_type" class="w-full bg-gray-900/50 border border-slate-600 rounded-md py-2 px-3">
                        ...
                        <div>
                            <label for="event-id" class="block text-sm font-medium text-slate-300 mb-1">Event Code (Opsional)</label>
                            <input type="text" id="event-id" name="event_id" class="w-full bg-gray-900/50 border border-slate-600 rounded-md py-2 px-3" placeholder="cth: BIRTHDAY24">
                        </div>
                        <div>
                            <label for="voucher-id" class="block text-sm font-medium text-slate-300 mb-1">Voucher Code (Opsional)</label>
                            <input type="text" id="voucher-id" name="voucher_id" class="w-full bg-gray-900/50 border border-slate-600 rounded-md py-2 px-3" placeholder="Masukkan kode voucher">
                        </div>
                        ...
                        <div id="reservation-fields" class="hidden space-y-4 pt-4 border-t border-slate-700/50">
                             ...
                             <div>
                                <label for="customer_name" class="block text-sm font-medium text-slate-300 mb-1">Nama Lengkap</label>
                                <input type="text" id="customer_name" name="customer_name" class="w-full bg-gray-900/50 border border-slate-600 rounded-md py-2 px-3" placeholder="Nama Anda">
                            </div>
                            <div>
                                <label for="reservation_date" class="block text-sm font-medium text-slate-300 mb-1">Tanggal & Waktu</label>
                                <input type="datetime-local" id="reservation_date" name="reservation_date" class="w-full bg-gray-900/50 border border-slate-600 rounded-md py-2 px-3">
                            </div>
                             <div>
                                <label for="number_of_guests" class="block text-sm font-medium text-slate-300 mb-1">Jumlah Tamu</label>
                                <input type="number" id="number_of_guests" name="number_of_guests" min="1" class="w-full bg-gray-900/50 border border-slate-600 rounded-md py-2 px-3" placeholder="cth: 4">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-900/50 p-4 rounded-lg flex flex-col">
                        ...
                        <div id="order-summary-list" class="flex-grow space-y-2 text-slate-300 text-sm pr-2 overflow-y-auto">
                ...
                <div class="mt-8 text-center">
                    <button type="submit" class="w-full md:w-auto bg-brand-accent hover:bg-brand-accent-dark text-black font-bold py-3 px-12 rounded-lg text-lg transition-transform duration-200 hover:scale-105">
                        Submit Pesanan
                    </button>
                </div>
            </section>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const updateOrder = () => {
                ...
                summaryList.innerHTML = summaryHTML || '<p class="text-slate-500 italic">Keranjang Anda kosong.</p>';
            };
            ...
            orderForm.addEventListener('submit', function(event) {
                ...
                confirmationHtml += `<hr class="my-2 border-slate-500"><p class="font-bold text-lg">Total: ${formatCurrency(totalPaymentInput.value)}</p></div>`;
